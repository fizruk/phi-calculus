...
import qualified LatexConstants as LC (ksi, phi, rho, upPhi, quad, lambda)
...
rule1 s =
  ...
  where
    ...
    getGmisForFreeAttributes ((A name) : xs) =
      [ADD v_i_x,
      ...
      s
        {
          ...
          -- add gmis produced by this rule
          gmis = gmisCurrent ++ gmis s
        }
